...
def main():
    ...
    if stock_symbol:
        try:
            ...
            fig2.add_trace(go.Scatter(x=stock_data.index, y=stock_data['MA200'], mode='lines', line=dict(dash='dot', color='green'), name='200DMA'))
            ...
            if selected_model == "Neural Network":
                if len(expected_input_shape) == 3:  # Model expects 3D input
                    x_pred = np.expand_dims(x_pred, axis=-1)
                elif len(expected_input_shape) == 4:  # Model expects 4D input (batch, time, features, channels)
